...
- Detections: 
  - A total of `r adults_count` (`r marked` marked and `r unmarked` unmarked) adult Delta Smelt have been detected this water year. 
  - The most recent detection(s) of `r last_catch_count` Delta Smelt occurred on `r format(last_catch_date, "%b %d, %Y")` in the `r last_catch_location` region
...
#| label: "table-catch-all"
#| tab-cap: "Delta Smelt Water Year Totals by Life Stage"
kable(
  ds_detail %>%
...
if (larval_ent_status != "relevant") {
  ...
  cat(glue("
  - **RVB temperature (3-day average)**: {RVB_temp} degC as of {format(env_last_date, '%b %d, %Y')}
...
- Include plot of CLC or link to CLC when relevant
