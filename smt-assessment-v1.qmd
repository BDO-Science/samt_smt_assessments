...
3. After the onset of spawning, have the following conditions occurred: JPF < 0 cfs, turbidity is $\ge$ 12 FNU in the south Delta, and PTM modeling indicates the action would avoid $\ge$ 5% entrainment of the Delta smelt population at facilities after 30 days?
...
(map_detections_lj <- ggplot() + 
    ...
    scale_shape_manual(values = c(1, 15, 17, 4, 5))+
    # scale_size(range = c(2, 2), breaks = c(1,1)) + 
    viridis::scale_fill_viridis(option = "turbo", discrete = TRUE) + 
    guides(fill = guide_legend(nrow = 3, byrow = TRUE)) +
    theme_bw() +
